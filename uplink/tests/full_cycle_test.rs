...
#[cfg(test)]
fn create_upload_list_download_delete() {
    ...
    let bucket_name = common::generate_name("full-cycle-create-upload-list-download-delete");
    ...
    assert!(
        it.find(|res| res.expect("list bucket iterator").name == &bucket_name)
            .is_some(),
    ...
    let mut object_it = project
        .list_objects(&bucket_name, None)
        .expect("list objects");

    assert!(
        (&object_it)
            .find(|res| res.expect("list object iterator").key == object_key)
            .is_some(),
    ...
    let downloaded_object_data = String::with_capacity(object_data.len());
    download
